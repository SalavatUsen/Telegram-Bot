...
async def welcome(message: Message):
    ...
    welcome_sticker = open('/home/sali/saitama.mp4', 'rb')
...
async def main_response(message: Message):
    ...
    if message.text == 'Забронировать столик ⏰':
        ...
        return await bot.send_message(
            chat_id=message.from_user.id,
            text="Выберите день бронирования:",
...
async def make_order(callback: CallbackQuery):
    ...
    if callback.data:
        ...
        return await bot.send_message(
            chat_id=callback.from_user.id,
            text='Пожалуйста потвердите свою личность!',
            reply_markup=btns.ask_phone()
        )
...
async def confirm_number_for_order(message: Message, state: FSMContext):
    if message.contact is not None:
        ...
        return await bot.send_message(
            chat_id=message.from_user.id,
            text='Потвердите свою геолокацию ☑',
...
async def confirm_location_for_order(message: Message):
    if message.location is not None:
        ...
        return await bot.send_message(
            chat_id=message.from_user.id,
            text='Ваш заказ успешно сохранён ☑'
        )
...
async def feedbacks(message: Message):
    ...
    if message.text == 'Оставить отзыв ✍':
        ...
        return await message.answer(
            text='*Пожалуйста оставьте мне ваш отзыв*',
            parse_mode=types.ParseMode.MARKDOWN
        )
...
async def insert_feedback(message: Message):
    if message.text == '' or message.text == ' ':
        ...
        return await message.answer(
            text='Потвердите свой номер телефона',
...
@dispatcher.message_handler(content_types=["text"], state='*')
